// SVG 路径来自 Material Icons，通过 data URI 渲染，效果与网页版 Icon.jsx 一致
const PATHS = {
  self_improvement: 'M12 2a3 3 0 1 0 0 6 3 3 0 0 0 0-6zm0 14c-5 0-6 2.5-6 2.5V20h12v-1.5S17 16 12 16zm-3.5-5.5C8.5 12.88 9 14 9 14h6s.5-1.12.5-3.5c0-1.5-.67-2.83-1.5-3.5-.5 1-1.5 1.5-2 1.5s-1.5-.5-2-1.5c-.83.67-1.5 2-1.5 3.5z',
  ...
  analytics: 'M4 20h3V10H4v10zm6.5 0h3V4h-3v16zM17 20h3v-7h-3v7z',
  ...
  accessibility_new: 'M12 2c1.1 0 2 .9 2 2s-.9 2-2 2-2-.9-2-2 .9-2 2-2zm8 7h-5v13h-2v-6h-2v6H9V9H4V7h16v2z',
  ...
  photo_camera: 'M12 15.2A3.2 3.2 0 0 1 8.8 12 3.2 3.2 0 0 1 12 8.8 3.2 3.2 0 0 1 15.2 12 3.2 3.2 0 0 1 12 15.2M9 2L7.17 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2h-3.17L15 2H9z',
  ...
  calendar_month: 'M19 3h-1V1h-2v2H8V1H6v2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V8h14v11zM7 10h5v5H7z',
...
Component({
  ...
  computed: {
    src() {
      const path = PATHS[this.data.name] || PATHS['person']
      return makeSvg(path, this.data.color)
    }
  },
  observers: {
    'name, color': function() {
      const path = PATHS[this.data.name] || PATHS['person']
      this.setData({ src: makeSvg(path, this.data.color) })
    }
  },
  lifetimes: {
    attached() {
