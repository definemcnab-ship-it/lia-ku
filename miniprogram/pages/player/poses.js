...
// 关键词 → 姿势，自上而下第一个命中生效
// 规则从具体到泛化，自上而下第一个命中生效。
// 只匹配动作名（move.name），不匹配 desc——desc 里常出现无关部位词导致误匹配。
const RULES = [
  // 器械/松解类（形态最独特，优先级最高）
  ...
  ['蚌式|侧卧', 'side_lying'],
  // 胸/肩前侧拉伸
  ['门框|胸小肌|胸肌拉伸|开胸', 'doorway'],
  // 靠墙类
  ['墙天使|靠墙|墙壁推|前锯肌', 'wall_angel'],
  // 拉的动作（面拉/划船/后缩/外旋/W肩）——水平拉，统一用面拉演示
  ['面拉|划船|肩胛后缩|外旋|W肩|后缩', 'band_pull'],
  // 俯卧 YTW 类
  ['YTW|Y-T|Y字|俯卧\\s*Y|俯卧', 'prone_ytw'],
  // 猫牛/胸椎/脊柱流动（四足跪姿）
  ['猫牛|四足|胸椎|脊柱波浪', 'quadruped'],
  // 足踝专项（注意：「脚跟滑动」是仰卧核心动作，不在此，走 supine）
  ['短足|足底|足弓|提踵|胫后|毛巾', 'foot'],
  // 小腿拉伸（腓肠/比目鱼/台阶站立拉伸）——用足踝演示
  ['腓肠|比目鱼|小腿', 'foot'],
  // 髋前侧/半跪/弓步/分腿蹲拉伸 + 北欧腘绳（跪姿前倾）
  ['弓步|半跪|沙发|分腿蹲|髂腰|股直|髋屈|髋前|北欧|腘绳', 'lunge'],
  // 蹲/硬拉（划船已移到 band_pull）
  ['深蹲|硬拉', 'squat'],
  // 单腿平衡/台阶/TKE 终末膝伸（站姿单腿）
  ['单腿|平衡|台阶|踏步|TKE|终末膝', 'balance'],
  // 颈侧向拉伸 / 梨状肌 4 字
  ['颈侧|侧颈|4字|梨状|肩胛提肌', 'neck_side'],
  // 颈线内收 / 深颈屈肌 / CCF
  ['内收|颈线|颌内收|颈屈|CCF|颈部热身', 'chin_tuck'],
  // 呼吸/骨盆/盆底/腹横肌（仰卧中性姿势）
  ['呼吸|枕下|90|仰卧|骨盆|盆底|腹横肌|滑动|步态|臀肌激活', 'supine_rest'],
  // 泛化拉伸兜底（站姿收下颌作最终中性形象）
  ['拉伸|舒展|松动', 'neck_side'],
]
